#pragma once
// #include "graphics/mesh/submesh/packedmesh.h" // originally for zenlib packed mesh modified to opengothic packedmesh class
...
namespace Utils
{
    class Exporter
    {
      ...
      {
        if(std::find(fileNames.begin(),fileNames.end(),file)!=fileNames.end()) // only export once
          return true;
        ...
        for (auto& v : mesh.vertices)
          fputs(("vt " + std::to_string(v.TexCoord.x) + " " + std::to_string(v.TexCoord.y) + "\n").c_str(), f);
        // Write normals
        for (auto& v : mesh.vertices)
          fputs(("vn " + std::to_string(v.Normal.x) + " " + std::to_string(v.Normal.y) + " " + std::to_string(v.Normal.z) + "\n").c_str(), f);
        fputs("s off\n", f);
        // Write faces
